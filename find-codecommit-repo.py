...
pattern = input(f"Provide partial repo name --> ")
        ...
        try:
            ...
            for page in page_iterator:
                for repo in page['repositories']:
                    ...
                    if re.search(pattern, repoName, re.IGNORECASE):
                        print(f"{repoName};{accountName};{rname}")

        except Exception as e:
          message_header = (f"Issue with listing codecommit repositories when using {profileName} profile in {rname} region, with error:\n{e}")
